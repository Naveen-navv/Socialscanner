...
function SubAdd({ onAdd }: { onAdd: (name: string, members: string) => void }) {
  ...
  const resolveMembers = async (normalizedName: string): Promise<string> => {
    ...
    const subSlug = normalizedName.replace(/^r\//i, "");
    ...
    const proxies = [
      `https://corsproxy.io/?url=${encodeURIComponent(`https://www.reddit.com/r/${subSlug}/about.json?raw_json=1`)}`,
      `https://api.allorigins.win/get?url=${encodeURIComponent(`https://www.reddit.com/r/${subSlug}/about.json`)}`,
    ];
    ...
    for (const proxyUrl of proxies) {
      try {
        ...
        if (r.ok) {
          ...
          const count = json?.data?.subscribers;
          if (count != null && Number.isFinite(Number(count))) return formatCount(Number(count));
        }
      } catch { /* try next */ }
    }
    ...
    try {
      ...
      let res = await fetch(`/api/subreddit-about?${qs}`, { signal: AbortSignal.timeout(10000) });
      if (!res.ok) {
      ...
      if (m !== undefined && m !== null && String(m).trim() !== "" && m !== "?") return String(m);
    } catch (err: any) {
      console.warn(`resolveMembers(${normalizedName}) server fallback failed:`, err);
    }
    return "?";
  };
...
export function Dashboard({ user, onLogout }: { user: any; onLogout: () => void }) {
  ...
  const [newFAType, setNewFAType] = useState<"lead" | "competitor">("lead");
  const LAST_SCAN_KEY = `ss_last_scan:${(user?.email || "").toLowerCase().trim()}`;
  ...
  const normalizeFocusArea = (focus: any) => ({
    ...focus,
    monitorType: focus?.monitorType || "lead",
    searchKeywords: focus?.searchKeywords || focus?.brandKeywords || [],
    ...
    negativeKeywords: focus?.negativeKeywords || [],
  });
  ...
  const quickScanIntel = async (id: string, sub: string) => {
    ...
    try {
      ...
      setIntel((prev) => prev.map((x) => x.id === id ? {
        ...x,
        ...profile,
        // Never overwrite a properly resolved member count with '?' from the backend
        members: (profile?.members && profile.members !== "?") ? profile.members : (x.members && x.members !== "?" ? x.members : (profile?.members || x.members || "?")),
        source: "live",
      } : x));
    } catch (err: any) {
      ...
      console.warn(`quickScanIntel(${sub}) failed:`, friendlyMessage);
      setIntel((prev) => prev.map((x) => x.id === id ? {
        ...x,
        scanStatus: "failed",
        scanMessage: friendlyMessage,
  ...
  const normalizedNegKeywords = useMemo(
    () => negativeKeywords.map((k) => k.toLowerCase().trim()).filter(Boolean),
    [negativeKeywords]
  );
  ...
  const visibleThreads = useMemo(() => {
    return threads.filter((t) => {
      if (!threadMatchesActiveConfig(t)) return false;
      if (normalizedNegKeywords.length > 0) {
        const text = `${t.title || ""} ${t.body || ""} ${t.replyTo?.text || ""}`.toLowerCase();
        if (normalizedNegKeywords.some((nk) => text.includes(nk))) return false;
      }
      return true;
    });
  }, [threads, fa, searchAll, normalizedToolTerms, normalizedNegKeywords]);
  ...
  function focusAreaMatchesThread(focus: any, thread: any, useSearchAll = false) {
    // Normalize subreddit names: strip "r/" prefix, lowercase for comparison
    const normSub = (s: string) => String(s || "").toLowerCase().trim().replace(/^r\//i, "");
    const subName = normSub(thread?.sub || "");
    const subredditMatch = useSearchAll || (focus.subreddits || []).some((sub: any) => normSub(sub?.name || sub) === subName);
    if (!subredditMatch) return false;
    // When intentDescription is set, subreddit match is enough — the AI intent filter
    // (filterByIntent) is the sole relevance gate (Layer 2). intentPatterns keyword
    // check only applies as a fallback when no intentDescription is configured.
    if (focus.intentDescription?.trim()) return true;
    const text = `${thread?.title || ""} ${thread?.body || ""}`.toLowerCase();
    const matchedPattern = String(thread?.matchedPattern || "").toLowerCase().trim();
    return (focus.intentPatterns || []).some((pattern: string) => {
  ...
  const fetchFromReddit = async (currentFa: any[], currentToolTerms: string[], useSearchAll = false, currentNegKeywords: string[] = []) => {
    if (!useSearchAll && !currentFa.length) return;
    setRefreshing(true);
    setScanError(null);
    try { localStorage.setItem(LAST_SCAN_KEY, String(Date.now())); } catch {}
    try {
      const allSubs = currentFa.flatMap(f => f.subreddits);
      const allPatterns = [...new Set(currentFa.flatMap(f => f.intentPatterns))];
      // Lead monitors contribute their extracted search keywords; competitor monitors contribute competitor names
      const allSearchKeywords = [...new Set([
        ...currentFa.filter(f => f.monitorType !== "competitor").flatMap(f => f.searchKeywords || []),
        ...currentFa.filter(f => f.monitorType === "competitor").flatMap(f => f.competitors || []),
      ])];
      const allNegKeywords = [...new Set(currentFa.flatMap(f => f.negativeKeywords || []))];
      const res = await fetch("/api/reddit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ subreddits: allSubs, searchKeywords: allSearchKeywords, intentPatterns: allPatterns, toolTerms: currentToolTerms, negativeKeywords: allNegKeywords, searchAll: useSearchAll }),
      });
      const data = await res.json();
  ...
  useEffect(() => {
    ...
    // Auto-scan once per 24 hours on app load. Manual Refresh always works.
    try {
      const lastScan = Number(localStorage.getItem(`ss_last_scan:${(user?.email || "").toLowerCase().trim()}`) || 0);
      if (Date.now() - lastScan > 24 * 60 * 60 * 1000) {
        const loadedFa = (initialData.fa || []).map((f: any) => ({ ...f, monitorType: f?.monitorType || "lead", searchKeywords: f?.searchKeywords || f?.brandKeywords || [] }));
        const loadedToolTerms = initialData.toolTerms || DEF_TOOL_TERMS;
        const loadedNegKws = initialData.negativeKeywords || [];
        setTimeout(() => void fetchFromReddit(loadedFa, loadedToolTerms, initialData.searchAll || false, loadedNegKws), 1500);
      }
    } catch {}
  ...
  const renderMonitor = () => {
    if (selFA) {
      ...
      const isCompetitor = f.monitorType === "competitor";
      const accentColor = isCompetitor ? C.orange : C.accent;
      return (<div>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
          ...
          <div>
            <h2 style={{ margin: 0, fontSize: 20, color: C.text, fontWeight: 700 }}>{f.name}</h2>
            <span style={{ fontSize: 12, color: accentColor }}>{isCompetitor ? "🔍 Competitor Monitor" : "🎯 Lead Monitor"} · Monitoring Setup</span>
          </div>
        </div>
        <div style={{ background: `${C.orange}10`, borderRadius: 8, padding: "10px 14px", marginBottom: 20, border: `1px solid ${C.orange}25`, fontSize: 12, color: C.muted }}>
          Changes save automatically. Go to <strong style={{ color: C.text }}>Leads → Clear → Refresh</strong> to apply updated filters to new results.
        </div>

        {/* SEARCH KEYWORDS — Lead monitors only */}
        {!isCompetitor && (
          <div style={{ marginBottom: 28 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
              <span style={{ fontSize: 13, fontWeight: 700, color: C.text, textTransform: "uppercase", letterSpacing: 1 }}>Search Keywords</span>
              <span style={{ fontSize: 12, color: C.muted }}>{(f.searchKeywords || []).length} terms</span>
            </div>
            <div style={{ fontSize: 12, color: C.muted, marginBottom: 10 }}>Sent to Reddit search. Write your intent description first, then click Regenerate.</div>
            <TagInput tags={f.searchKeywords || []} setTags={t => updateFA(f.id, { searchKeywords: t })} placeholder="Add keyword..." color={C.accent} />
            <button
              onClick={async () => {
                if (!f.intentDescription?.trim() || generatingKeywords) return;
                setGeneratingKeywords(true);
                const kws = await generateSearchKeywords(f.intentDescription);
                if (kws.length) updateFA(f.id, { searchKeywords: kws });
                setGeneratingKeywords(false);
              }}
              disabled={!f.intentDescription?.trim() || generatingKeywords}
              style={{ marginTop: 8, background: generatingKeywords || !f.intentDescription?.trim() ? C.border : `${C.accent}20`, color: generatingKeywords || !f.intentDescription?.trim() ? C.muted : C.accent, border: `1px solid ${!f.intentDescription?.trim() ? C.border : C.accent}40`, borderRadius: 8, padding: "7px 14px", cursor: !f.intentDescription?.trim() || generatingKeywords ? "not-allowed" : "pointer", fontWeight: 600, fontSize: 12 }}
            >{generatingKeywords ? "Generating..." : "↺ Regenerate from Intent"}</button>
          </div>
        )}

        {/* COMPETITORS — competitor monitors only */}
        {isCompetitor && (
          <div style={{ marginBottom: 28 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: C.text, textTransform: "uppercase", letterSpacing: 1 }}>Competitors</span>
                <span style={{ fontSize: 11, color: C.orange, fontWeight: 600 }}>These are your search terms</span>
              </div>
              <span style={{ fontSize: 12, color: C.muted }}>{f.competitors.length} tracked</span>
            </div>
            <TagInput tags={f.competitors} setTags={t => updateFA(f.id, { competitors: t })} placeholder="Competitor..." color={C.orange} />
          </div>
        )}
        ...
        <div style={{ marginBottom: 28 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: C.text, textTransform: "uppercase", letterSpacing: 1 }}>Subreddits</span>
            <span style={{ fontSize: 12, color: C.muted }}>{f.subreddits.length} active</span>
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 10 }}>
            {f.subreddits.map((s: any) => (
              <Badge key={s.name} color={C.blue} onRemove={() => updateFA(f.id, { subreddits: f.subreddits.filter((x: any) => x.name !== s.name) })}>
                {s.name} <span style={{ fontSize: 11, color: s.members === "?" ? C.orange : C.muted }} title={s.members === "?" ? "Subscriber count unknown — this subreddit will still be scanned" : undefined}>{s.members}</span>
              </Badge>
            ))}
          </div>
          <SubAdd onAdd={(n, m) => { if (!f.subreddits.find((s: any) => s.name.toLowerCase() === n.toLowerCase())) { updateFA(f.id, { subreddits: [...f.subreddits, { name: n, members: m }] }); addIntelSubreddit(n, m); } }} />
        </div>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <div style={{ fontSize: 12, color: C.muted, marginBottom: 10 }}>
            {isCompetitor ? "Only posts showing frustration or switching intent reach your leads." : "Only posts matching this description reach your leads."}
          </div>
          <textarea
            value={f.intentDescription || ""}
            onChange={e => updateFA(f.id, { intentDescription: e.target.value })}
            placeholder={isCompetitor
              ? "e.g. Person frustrated with or switching from a budgeting app. Looking for alternatives."
              : "e.g. Someone struggling to track salary, EMIs, or household expenses and looking for a simpler budgeting method"}
            rows={3}
        ...
        <div style={{ marginBottom: 28 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: C.text, textTransform: "uppercase", letterSpacing: 1 }}>Exclude Keywords</span>
            <span style={{ fontSize: 12, color: C.muted }}>{(f.negativeKeywords || []).length} terms</span>
          </div>
          <div style={{ fontSize: 12, color: C.muted, marginBottom: 10 }}>Posts containing these words are hidden from leads.</div>
          <TagInput tags={f.negativeKeywords || []} setTags={t => updateFA(f.id, { negativeKeywords: t })} placeholder='e.g. "real estate", "investing tips"...' color={C.danger} />
        </div>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <button
            ...
            style={{ background: testLoading || !testPostText.trim() || !f.intentDescription?.trim() ? C.border : accentColor, color: "#fff", border: "none", borderRadius: 8, padding: "9px 20px", cursor: testLoading || !testPostText.trim() || !f.intentDescription?.trim() ? "not-allowed" : "pointer", fontWeight: 600, fontSize: 13, marginBottom: 12 }}
          >
          ...
          {testResult && (
            <div style={{ padding: "12px 16px", borderRadius: 8, background: testResult.status === "error" ? `${C.orange}15` : testResult.pass ? `${C.green}15` : `${C.danger}15`, border: `1px solid ${testResult.status === "error" ? C.orange : testResult.pass ? C.green : C.danger}` }}>
              ...
              {!!testResult.matchedIntent && <div style={{ marginTop: 10, fontSize: 12, color: C.text }}><span style={{ fontWeight: 700 }}>Matched intent:</span> {testResult.matchedIntent}</div>}
              {!!testResult.matchedKeywords?.length && <div style={{ marginTop: 8, fontSize: 12, color: C.text }}><span style={{ fontWeight: 700 }}>Matched keywords:</span> {testResult.matchedKeywords.join(", ")}</div>}
            </div>
          )}
        </div>
      </div>);
    ...
    const createMonitor = () => {
      ...
      const isComp = newFAType === "competitor";
      setFa(p => [...p, {
        id: `fa_${Date.now()}`,
        name: newFAName.trim(),
        icon: isComp ? "🔍" : "🎯",
        monitorType: newFAType,
        searchKeywords: [],
        competitors: [],
        subreddits: [],
        intentDescription: isComp ? "Person frustrated with or thinking of switching from a budgeting or expense tracking app. Looking for alternatives. May mention specific pain points like price, missing features, or app shutting down." : "",
        intentPatterns: isComp ? ["frustrated", "switching", "alternative", "stopped using", "giving up", "cancelled", "too expensive"] : ["best", "vs", "alternative", "review"],
      }]);
      setNewFAName("");
      setShowNewFA(false);
      setNewFAType("lead");
    ...
    return (<div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
        <div><h2 style={{ margin: 0, fontSize: 22, color: C.text, fontWeight: 700 }}>Monitor</h2><p style={{ margin: "4px 0 0", fontSize: 13, color: C.muted }}>Two monitor types: Lead (find people with a problem) and Competitor (find people switching apps)</p></div>
        <button onClick={() => setShowNewFA(true)} style={{ background: C.accent, color: "#fff", border: "none", borderRadius: 8, padding: "10px 20px", cursor: "pointer", fontWeight: 600, fontSize: 14 }}>+ New</button>
      </div>
      {showNewFA && (
        <div style={{ background: C.card, borderRadius: 12, padding: 20, marginBottom: 20, border: `1px solid ${C.border}` }}>
          <div style={{ fontSize: 12, color: C.muted, marginBottom: 8, fontWeight: 600, textTransform: "uppercase", letterSpacing: 1 }}>Monitor type</div>
          <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
            <button onClick={() => setNewFAType("lead")} style={{ flex: 1, padding: "10px 12px", borderRadius: 8, border: `2px solid ${newFAType === "lead" ? C.accent : C.border}`, background: newFAType === "lead" ? `${C.accent}15` : C.bg, color: newFAType === "lead" ? C.accent : C.muted, cursor: "pointer", fontWeight: 600, fontSize: 13 }}>🎯 Lead Monitor<div style={{ fontSize: 11, fontWeight: 400, marginTop: 2 }}>Find people with a problem</div></button>
            <button onClick={() => setNewFAType("competitor")} style={{ flex: 1, padding: "10px 12px", borderRadius: 8, border: `2px solid ${newFAType === "competitor" ? C.orange : C.border}`, background: newFAType === "competitor" ? `${C.orange}15` : C.bg, color: newFAType === "competitor" ? C.orange : C.muted, cursor: "pointer", fontWeight: 600, fontSize: 13 }}>🔍 Competitor Monitor<div style={{ fontSize: 11, fontWeight: 400, marginTop: 2 }}>Find people switching apps</div></button>
          </div>
          <input value={newFAName} onChange={e => setNewFAName(e.target.value)} placeholder="Monitor name..." onKeyDown={e => { if (e.key === "Enter") createMonitor(); }} style={{ width: "100%", padding: "10px 14px", background: C.bg, border: `1px solid ${C.border}`, borderRadius: 8, color: C.text, fontSize: 14, marginBottom: 12, boxSizing: "border-box" }} />
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={createMonitor} style={{ background: newFAType === "competitor" ? C.orange : C.accent, color: "#fff", border: "none", borderRadius: 8, padding: "8px 20px", cursor: "pointer", fontWeight: 600 }}>Create</button>
            <button onClick={() => { setShowNewFA(false); setNewFAName(""); setNewFAType("lead"); }} style={{ background: "transparent", color: C.muted, border: `1px solid ${C.border}`, borderRadius: 8, padding: "8px 20px", cursor: "pointer" }}>Cancel</button>
          </div>
        </div>
      )}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 16 }}>
        {fa.map(f => {
          const isComp = f.monitorType === "competitor";
          const cardAccent = isComp ? C.orange : C.accent;
          return (
            <div key={f.id} onClick={() => setSelFA(f.id)} style={{ background: C.card, borderRadius: 12, padding: 20, cursor: "pointer", border: `1px solid ${C.border}`, borderLeft: `4px solid ${cardAccent}`, transition: "transform 0.15s", position: "relative" }} onMouseEnter={e => (e.currentTarget.style.transform = "translateY(-2px)")} onMouseLeave={e => (e.currentTarget.style.transform = "none")}>
              <button onClick={e => { e.stopPropagation(); setFa(p => p.filter(x => x.id !== f.id)); }} style={{ position: "absolute", top: 10, right: 12, background: "none", border: "none", color: C.muted, cursor: "pointer", opacity: 0.3 }} onMouseEnter={e => (e.currentTarget.style.opacity = "1")} onMouseLeave={e => (e.currentTarget.style.opacity = "0.3")}>x</button>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                <span style={{ fontSize: 24 }}>{f.icon}</span>
                <div><h3 style={{ margin: 0, fontSize: 18, color: C.text, fontWeight: 700 }}>{f.name}</h3><span style={{ fontSize: 11, color: cardAccent, fontWeight: 600 }}>{isComp ? "Competitor Monitor" : "Lead Monitor"}</span></div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, fontSize: 12, marginTop: 10 }}>
                {!isComp && <div style={{ color: C.muted }}><span style={{ color: C.accent, fontWeight: 700 }}>{(f.searchKeywords || []).length}</span> search keywords</div>}
                <div style={{ color: C.muted }}><span style={{ color: C.orange, fontWeight: 700 }}>{f.competitors.length}</span> competitors</div>
                <div style={{ color: C.muted }}><span style={{ color: C.blue, fontWeight: 700 }}>{f.subreddits.length}</span> subreddits</div>
                <div style={{ color: C.muted }}><span style={{ color: C.green, fontWeight: 700 }}>{f.intentDescription ? "✓" : "–"}</span> intent set</div>
              </div>
            </div>
          );
        })}
      </div>
      {fa.length === 0 && !showNewFA && <div style={{ textAlign: "center", padding: 60, color: C.muted }}>No monitors yet. Create a Lead or Competitor monitor to get started.</div>}
    </div>);
  };
  ...
  const renderWorkspace = () => {
    ...
    const regen = async (tone?: string, len?: string, target?: "comment" | "post") => {
    ...
    const switchReplyTarget = (nextTarget: "comment" | "post") => {
      ...
      if (!isP) {
        void regen(undefined, undefined, nextTarget);
      }
    };
      ...
      <div style={{ display: "grid", gridTemplateColumns: "5fr 7fr", gap: 16 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div><div style={{ fontSize: 10, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>Replying To</div>
            <div style={{ display: "flex", gap: 4, marginBottom: 10, background: C.bg, borderRadius: 8, padding: 3 }}>
              ...
              <button onClick={() => switchReplyTarget("comment")} disabled={!t.replyTo} style={{ flex: 1, padding: "7px 0", borderRadius: 6, border: "none", cursor: t.replyTo ? "pointer" : "not-allowed", fontSize: 12, fontWeight: 600, background: replyTarget === "comment" ? C.accent : "transparent", color: replyTarget === "comment" ? "#fff" : C.muted, opacity: t.replyTo ? 1 : 0.4 }}>Top Comment</button>
            </div>
  ...
  const renderSettings = () => {
    ...
    return (<div>
      ...
      {settingsTab === "intelligence" && <>
        ...
        {/* Derive displayed list from ALL subreddits in all focus areas — FA is the source of truth.
             Intel entries added directly in this tab (not in any FA) are also shown. */}
        {(() => {
          const dateLabel = new Date().toLocaleDateString("en-US", { month: "short", day: "numeric" });
          const blank = (name: string, members: string = "?") => ({
            id: `stub_${name}`, sub: name, members,
            lastScanned: "Not scanned yet", confidence: 0,
            rules: ["Not scanned yet"],
            bestTimes: { peak: "—", avoid: "—" },
            toneProfile: { preferred: "—", avoid: "—" },
            topFormats: [],
            whatWorks: [{ insight: "Run a scan to generate insights", score: 0 }],
            whatFails: [{ insight: "Run a scan to generate insights", score: 0 }],
            keywords: { positive: [], negative: [] },
            modStrictness: 0,
            sentimentBreakdown: { positive: 33, neutral: 34, negative: 33 },
            learningLog: [{ date: dateLabel, entry: "Added from Monitor setup" }],
            scanStatus: "pending" as const,
            scanMessage: "Click to scan"
          });
          // Collect unique subs from all focus areas
          const faSubs = new Map();
          for (const f of fa) {
            for (const s of (f.subreddits || [])) {
              if (!faSubs.has(s.name)) faSubs.set(s.name, s);
            }
          }
          // Helper: pick the best available member count, ignoring '?'
          const bestMembers = (a: string | undefined, b: string | undefined, subName: string): string => {
            // Check SUBS_DB first for hardcoded values
            const dbHit = SUBS_DB.find(s => s.name.toLowerCase() === subName.toLowerCase());
            if (dbHit) return dbHit.m;
            // Then prefer whichever of a/b is not '?' or empty
            if (a && a !== "?") return a;
            if (b && b !== "?") return b;
            return "?";
          };
          // Build display list: FA subs first (with intel data if available), then intel-only subs
          const intelMap = new Map(intel.map(s => [s.sub, s]));
          const displayList = [];
          for (const [name, faSub] of faSubs) {
            const intelEntry = intelMap.get(name);
            const members = bestMembers(intelEntry?.members, faSub.members, name);
            displayList.push(intelEntry ? { ...intelEntry, members } : blank(name, members));
          }
          // Also add subs that are only in intel (added directly in this tab)
          ...
          <div style={{ fontSize: 12, color: C.muted }}>Tone: <span style={{ color: si.id.startsWith("stub_") ? C.muted : C.green }}>{si.toneProfile.preferred}</span></div>
        </div>)}</div>;
        })()}
      </>}
    </div>);
